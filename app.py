...
# === إعداد Firebase ===
cred = credentials.Certificate("touirismapp-firebase-adminsdk-fbsvc-4fa43a5357.json")  # عدّل الاسم حسب ملفك
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://touirismapp-default-rtdb.firebaseio.com/'  # عدّلها حسب مشروعك
})
...
def add_place():
    ...
    if request.method == 'POST':
        ...
        link_photo = request.form['link_photo'].strip()
        ...
        # تحقق إذا كان المكان موجود مسبقًا
        if ref.get() is not None:
            return render_template("add_place.html", error="❗المكان موجود مسبقًا.")
...
def update_place():
    ...
    name_country = data.get('name_country', '').strip()
    ...
    try:
        ...
        ref.update({
            ...
            'description': description,
            'link_photo': link_photo,
            'flag': "img/flags/"+name_country+".png",
            'lat': lat,
            'lng': lng,
            'name_country': name_country,
        })
