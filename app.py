...
def add_place():
    ...
    if request.method == "POST":
        name_place = request.form["name_place"].strip()
        name_country = request.form["name_country"].strip()
        description = request.form["description"].strip()
        lat = request.form.get("lat", None)
        lng = request.form.get("lng", None)

        # حفظ الصورة
        photo_file = request.files.get("link_photo")
        filename = None
        ...
        place = Place(
            ...
            link_photo=filename,
            flag=f"img/flags/{name_country}.png",
            link_pa=f"place/{name_place.replace(' ', '_').lower()}",
            details_url=f"admin/{name_place.replace(' ', '_').lower()}"
        )
...
def delete_place(place_id):
    ...
    # لو الصورة موجودة امسحها
    if place.link_photo:
        photo_path = os.path.join(app.config["UPLOAD_FOLDER"], place.link_photo)
...
def update_place(place_id):
    ...
    if request.method == "POST":
        ...
        place.lat = request.form["lat"]
        place.lng = request.form["lng"]

        # حفظ الصورة
        photo_file = request.files.get("link_photo")
        if photo_file and photo_file.filename != "":
            old_photo = place.link_photo  # خزن الصورة القديمة قبل التغيير
            filename = secure_filename(photo_file.filename)
            photo_path = os.path.join(app.config["UPLOAD_FOLDER"], filename)
            photo_file.save(photo_path)

            place.link_photo = filename

            # حذف الصورة القديمة بعد التأكد إنها موجودة
            if old_photo:
